...
- **WebSocket capture:** Chrome emits WS lifecycle/frame events as part of the `Network` domain (no separate CDP domain to enable). Connection lifecycle (`network.ws_open`, `network.ws_handshake`, `network.ws_error`, `network.ws_close`) is always captured in full — cheap and high-signal. Frame payloads (`network.ws_frame`) follow the same sampling philosophy as HTTP bodies but *per connection* rather than per endpoint: the first `WS_FRAME_SAMPLE_LIMIT` (5) frames per direction per socket get a full (redacted) payload, later frames are just counted/sized (`_wsConnections` map in `background.js`) — this avoids flooding capture on a live telemetry/status socket that pushes many frames per second. `network.ws_close` always reports total frame counts/bytes for the connection even when individual payloads were sampled out, so volume is never lost. `network.ws_error` is treated as a triage anchor in `filter.js`, same tier as `network.failure`.
  - **Black-box tail buffer:** because head-sampling means the frames right before a failure are the ones most likely to have aged out of the sample, every connection also keeps a small ring buffer (`WS_TAIL_BUFFER_SIZE` = 5, raw/unredacted, `conn.tailFrames`) of its most recent frames. On `ws_error` or `ws_close`, if any frame was ever omitted for that connection (`conn.hadOmittedFrame`), the buffer is flushed as a `network.ws_tail` event with full (redacted) payloads. Redaction only runs on the handful of frames actually flushed, not on every frame, so the steady-state cost on a chatty socket is unchanged (`flushWsTail()` in `background.js`).
  - **Dashboard "WebSockets" tab:** `server/public/app.js` (`buildWsConnections`/`renderWsConnections`) groups the flat `network.ws_*` event stream by `request_id` into one expandable row per connection (open→close, with frame timeline and recovered tail frames inline), instead of leaving WS traffic interleaved with everything else in the Raw Events tab.
